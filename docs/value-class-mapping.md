...
- **Phase 2 — eligible case classes, opt-in (implemented; `valhalla-annotations` + `ValueClassTransformer`).** A dependency-free runtime annotation `@au.id.zaugg.valhalla.ValueClass` (RUNTIME-retained, read from `RuntimeVisibleAnnotations`) marks a final class of final fields for promotion; the agent then makes *all* its fields strict and clears `ACC_IDENTITY`, gated by the eligibility filter (fields final and set before `super()`, float/double off unless `allowFloating=true` per type). Opt-in rather than inferred because dropping identity is not whole-program-sound to assume.
  - **Superclass rule.** A value class may only extend `Object` or an **abstract `@ValueClass`** *value super class*, which JEP 401 requires to be stateless. The same annotation on an abstract class marks such a super and is checked for **zero instance fields** (verified: javac rejects a field on an abstract value class); the agent resolves a non-`Object` superclass's bytes (via the classloader at load time, the classes dir offline) and validates the whole chain up to `Object`. A case class extending a non-annotated or stateful parent is excluded — promoting it would make a value class extend an identity class.
  - Verified on the EA JVM: annotated multi-field case classes compare `==` by state with `equals`/`toString` retained; subclasses of an abstract `@ValueClass` super load and `instanceof` correctly; field-bearing abstracts and non-`@ValueClass`-parent subclasses are gated out; un-annotated classes stay identity.
