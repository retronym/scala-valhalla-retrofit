...
- **Phase 2 — eligible case classes, opt-in (implemented; `valhalla-annotations` + `ValueClassTransformer`).** A dependency-free runtime annotation `@au.id.zaugg.valhalla.ValueClass` (RUNTIME-retained, read from `RuntimeVisibleAnnotations`) marks a final class of final fields for promotion; the agent then makes *all* its fields strict and clears `ACC_IDENTITY`, gated by the eligibility filter (fields final and set before `super()`, float/double off unless `allowFloating=true` per type). Opt-in rather than inferred because dropping identity is not whole-program-sound to assume.
  ...
  - **Inner-class rule.** A value class may **not enclose a non-static inner member class** (an enclosing value instance has no identity to capture — verified: `ClassFormatError` otherwise, e.g. `scala.Option`'s `WithFilter`). And when promoting, the agent fixes the `InnerClasses` attribute so identity nested classes carry `ACC_IDENTITY`, matching javac (`0x28` STATIC|IDENTITY for an identity class nested in a value class) — otherwise the entry is rejected.
...
- **Phase 2 (config) — classes we cannot annotate (implemented; `ValueClassConfig`).** The same promotion driven by external config instead of the annotation, for stdlib/library types: a **value-class list** (concrete → promote, abstract → value super), a **finalize list** (add `ACC_FINAL` to a class whose author forgot it, then promote), an **allow-non-final** mode (blanket skip of the not-final gate), and **staticize-inner** (convert a named non-static inner class to static by flipping `ACC_STATIC` on its `InnerClasses` entries everywhere). Loadable from a `.properties` file or inline agent/CLI options. Honest results against `scala-library`: `scala.util.Either`/`Left`/`Right` promote and load (`Left(e) == Left(e)` is `true`); `Range.Inclusive`/`Exclusive` are gated (`Range` is stateful). `scala.Option`/`Some` are gated by default (Option encloses the non-static inner `WithFilter`) but **become promotable with `staticize-inner=scala.Option$WithFilter`** — `WithFilter` already takes its outer `Option` as an explicit constructor argument, so making it static needs no code change, and a `for`-comprehension guard (compiled to `Option.withFilter`) still works (`Some(42)`). The mechanism exists; what is *sound* is determined per class by the same eligibility filter, with a precise skip reason for the rest.
