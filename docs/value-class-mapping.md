...
- **Phase 2 — eligible case classes, opt-in (implemented; `valhalla-annotations` + `ValueClassTransformer`).** A dependency-free runtime annotation `@au.id.zaugg.valhalla.ValueClass` (RUNTIME-retained, read from `RuntimeVisibleAnnotations`) marks a final class of final fields for promotion; the agent then makes *all* its fields strict and clears `ACC_IDENTITY`, gated by the same eligibility filter (Object superclass, fields set before `super()`, float/double off unless `allowFloating=true` per type). Opt-in rather than inferred because dropping identity is not whole-program-sound to assume. Verified on the EA JVM: annotated multi-field case classes compare `==` by state with `equals`/`toString` retained; un-annotated classes stay identity.
