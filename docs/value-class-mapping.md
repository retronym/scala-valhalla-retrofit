...
- **Phase 2 — eligible case classes, opt-in (implemented; `valhalla-annotations` + `ValueClassTransformer`).** A dependency-free runtime annotation `@au.id.zaugg.valhalla.ValueClass` (RUNTIME-retained, read from `RuntimeVisibleAnnotations`) marks a final class of final fields for promotion; the agent then makes *all* its fields strict and clears `ACC_IDENTITY`, gated by the eligibility filter (fields final and set before `super()`, float/double off unless `allowFloating=true` per type). Opt-in rather than inferred because dropping identity is not whole-program-sound to assume.
  ...
  - **Inner-class rule.** A value class *may* have a non-static inner member class — javac compiles one without complaint (no source-level error). The only requirement is that an identity nested class carry `ACC_IDENTITY` in its `InnerClasses` entry (javac emits `0x20` non-static / `0x28` static; the JVM rejects e.g. `scala.Option`'s `WithFilter` at `0x1` otherwise). The agent applies this **transparently** — on the promoted class and on any class whose `InnerClasses` entry names a value class as the outer (so a value class's own nested classes, like `WithFilter`, are fixed even though they aren't promoted), leaving the inner non-static.
...
- **Phase 2 (config) — classes we cannot annotate (implemented; `ValueClassConfig`).** The same promotion driven by external config instead of the annotation, for stdlib/library types: a **value-class list** (concrete → promote, abstract → value super), a **finalize list** (add `ACC_FINAL` to a class whose author forgot it, then promote), and an **allow-non-final** mode (blanket skip of the not-final gate). Loadable from a `.properties` file or inline agent/CLI options. Honest results against `scala-library`: `scala.util.Either`/`Left`/`Right` and `scala.Option`/`Some` promote and load (`Left(e) == Left(e)` and `Some(1) == Some(1)` are `true`); `Range.Inclusive`/`Exclusive` are gated (`Range` is stateful). `Option`/`Some` promote even though `Option` has a non-static inner class (`WithFilter`) — that is not a value-class restriction (javac allows it); the inner just needs `ACC_IDENTITY` in its `InnerClasses` entry, which the agent applies transparently, leaving `WithFilter` non-static (a `for`-comprehension guard compiled to `Option.withFilter` still yields `Some(42)`). The mechanism exists; what is *sound* is determined per class by the same eligibility filter, with a precise skip reason for the rest.
